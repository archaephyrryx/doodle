...
impl Expr {
    ...
    fn infer_type(&self, scope: &TypeScope<'_>) -> AResult<ValueType> {
        match self {
            ...
            Expr::Lambda(..) => Err(anyhow!("unexpected lambda")),

            Expr::IntRel(_rel, x, y) => match (x.infer_type(scope)?, y.infer_type(scope)?) {
                (ValueType::Ground(g1), ValueType::Ground(g2)) if g1 == g2 && g1.is_numeric() => Ok(ValueType::Ground(GroundType::Bool)),
                (x, y) => Err(anyhow!("mismatched operands {x:?}, {y:?}")),
            },
            Expr::Arith(_arith, x, y) => match (x.infer_type(scope)?, y.infer_type(scope)?) {
                (ValueType::Ground(g1), ValueType::Ground(g2)) if g1 == g2 && g1.is_numeric() => Ok(ValueType::Ground(g1)),
                (x, y) => Err(anyhow!("mismatched operands {x:?}, {y:?}")),
            },
            ...
            Expr::U16Be(bytes) => {
                ...
                match _t.unwrap_tuple_type().as_slice() {
                    [ValueType::Ground(GroundType::U8), ValueType::Ground(GroundType::U8)] =>
                        Ok(ValueType::Ground(GroundType::U16)),
                    _ => Err(anyhow!("bad cast: U16Be @ {_t:?}")),
                }
            }
            Expr::U16Le(bytes) => {
                let _t = bytes.infer_type(scope)?;
                match _t.unwrap_tuple_type().as_slice() {
                    [ValueType::Ground(GroundType::U8), ValueType::Ground(GroundType::U8)] =>
                        Ok(ValueType::Ground(GroundType::U16)),
                    _ => Err(anyhow!("bad cast: U16Le @ {_t:?}")),
                }
            }
            Expr::U32Be(bytes)  => {
                let _t = bytes.infer_type(scope)?;
                match _t.unwrap_tuple_type().as_slice() {
                    ...
                    _ => Err(anyhow!("bad cast: U32Be @ {_t:?}")),
                }
            }
            Expr::U32Le(bytes) => {
                let _t = bytes.infer_type(scope)?;
                match _t.unwrap_tuple_type().as_slice() {
                    ...
                    _ => Err(anyhow!("bad cast: U32Le @ {_t:?}")),
                }
            }
            Expr::SeqLength(seq) =>
                match seq.infer_type(scope)? {
                    ValueType::Seq(_t) => Ok(ValueType::Ground(GroundType::U32)),
                    other => Err(anyhow!("SeqLength expected Seq, found {other:?}")),
                }
            Expr::SubSeq(seq, start, length) =>
                match seq.infer_type(scope)? {
                    ValueType::Seq(t) => {
                        ...
                        if start_type != ValueType::Ground(GroundType::U32) {
                            return Err(
                                anyhow!("SubSeq `start` param: expected U32, found {start_type:?}")
                            );
                        }
                        if length_type != ValueType::Ground(GroundType::U32) {
                            return Err(
                                anyhow!("SubSeq `length` param: expected U32, found {start_type:?}")
                            );
                        }
                        Ok(ValueType::Seq(t))
                    }
                        ...
                        match seq.infer_type(scope)? {
                            ValueType::Seq(t) => {
                                ...
                                match expr.infer_type(&child_scope)? {
                                    ...
                                    other => Err(anyhow!("FlatMap lambda output: expected Seq, found {other:?}")),
                                }
                            }
            ...
            Expr::Dup(count, expr) => {
                if count.infer_type(scope)? != ValueType::Ground(GroundType::U32) {
